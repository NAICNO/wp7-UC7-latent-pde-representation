...
The PDE family under study is the **steady-state convection-diffusion equation**
in two spatial dimensions on [-1, 1]^2:

```
-epsilon * Laplacian(u) + b(x) . grad(u) = 0
```

where **b(x)** is a divergence-free advection field derived from a polynomial
streamfunction. Different random streamfunction coefficients produce different
...
Full tutorial built with Sphinx and deployed to GitLab Pages:

...
- Start Jupyter with explicit IP binding: `--ip=0.0.0.0`
...
The project solves the steady-state convection-diffusion equation in 2D:

```
-epsilon * Laplacian(u) + b(x) . grad(u) = 0    on [-1, 1]^2
```

with Dirichlet boundary conditions and a divergence-free advection field **b(x)** derived from a streamfunction. The streamfunction is parameterized by a small number of coefficients (typically 10-20), making it a natural low-dimensional representation of the PDE family.
